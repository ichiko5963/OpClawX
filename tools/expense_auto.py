...
"""
経費領収書自動処理（完全版）
Telegram画像 → Claude分析 → Drive保存 → スプレッドシート挿入
"""
import sys
import json
import requests
...
def get_expense_sheet_id(access_token):
    """経費領収書シートのIDを取得"""
...
def insert_at_row3(access_token, expense_data, drive_url):
    ...
    expense_sheet_id = get_expense_sheet_id(access_token)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: expense_auto.py <image_path> <expense_json>")
        sys.exit(1)
