#!/usr/bin/env python3
"""
LINE公式アカウント使い方マニュアル（Google Docs版）- 更新版
修正機能・新列構造対応
"""
...
def update_document(doc_id, access_token):
    """既存ドキュメントを更新"""
    ...
    requests.post(batch_url, headers={**headers, 'Content-Type': 'application/json'}, 
...
"""
    ...
    # 本文挿入
    full_text = main_text + example_message + correction_section
    ...
    # 一括更新
    response = requests.post(batch_url, headers={**headers, 'Content-Type': 'application/json'}, 
                            json={'requests': content_requests})
...
def main():
    ...
    print("🔄 LINE Botマニュアル更新中...")
    
    access_token = get_access_token()
    result = update_document(doc_id, access_token)
